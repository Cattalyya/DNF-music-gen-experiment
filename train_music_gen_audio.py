...
def get_notes():
    notes = []
    with open("data/notes_original", 'rb') as filepath:
...
def parse_notes():
    ...
    for file in glob.glob("training_midis/*.midi"):
...
def prepare_sequences(notes, n_vocab):
    ...
    print("len(notes) - seq_len: ", len(notes) - seq_len)
    ...
    print("(n_patterns, seq_len, 1): ", (n_patterns, seq_len, 1))
...
def create_network_medium(net_in, n_vocab):
...
def create_network_small(net_in, n_vocab):
    """create the structure of neural net"""
    model = Sequential()
    model.add(LSTM(
        64,
...
def train_network():
    ...
    print(len(notes), n_vocab)
    net_in, net_out = prepare_sequences(notes, n_vocab)

    model = create_network_small(net_in, n_vocab)
